...
```
#####Code Review:
see attached cs 340 code review zip file

 ```
 
 ```
#####Software Design and Engineering:
  The interactive database and data table was not working this update to the old code should allow the data table to be used. Originally, before the inclusion of the CRUD document, the HTML dashboard laded and presented the user with a username and password box to login. Once I incorporated the crud file in jupyter and updated the html format code something  broke. The format I will use is simple and almost a basic default setting for the client. I wont try and get to fancy with style, height and size adjustments or even color. The Jupyter ipynb also needs to be revised as the previous code was formatted with incorrect input data. This caused my html dashboard to load but not display any of the database.  The goal is to get the dashboard to load the correct UI features and connect to the database to display the search filters and pie graph. This is needed to provide a great UI and deliver a final product that meets the requirements of the customer.
